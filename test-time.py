...
def evaluate(cfg):
    ...
    #base_model = torch.load('eata_surgical.pth').cuda()
    ...
    elif cfg.MODEL.ADAPTATION == "hrtta":
        model, param_names = setup_hrtta(base_model, num_classes, cfg)
    else:
    ...
    elif cfg.CORRUPTION.DATASET in {"imagenet_r"}:
        dom_names_all = ['renditions']
    else:
    ...
    for i_dom, domain_name in enumerate(dom_names_loop):
        ...
        for severity in severities:
            ...
            for epoch in range(cfg.TEST.EPOCH):
                ...
                # torch.save(model, "eata_surgical.pth")
            accs.append(acc)
            logger.info(
